...
# Vendored version: Build kinematic tree from hardcoded Mixamo hierarchy
# (Original used blender_utils to parse FBX, but we don't need bpy dependency)

# Mixamo skeleton parent relationships (child -> parent)
# This is the standard Mixamo hierarchy used by Make-It-Animatable
_MIXAMO_PARENTS = {
    "mixamorig:Hips": None,
...
def _build_kinematic_tree(bones_idx_dict: dict[str, int]) -> Joint:
    ...
    # Create all joints first (without parent/children links)
    joints_dict: dict[str, Joint] = {}
    for name, idx in bones_idx_dict.items():
        # Use object.__new__ to create without calling __init__ (frozen dataclass)
...
def animate_char(
    ...
    anim_path: str,
    template_dict: dict[str, int],
    load_fn: Callable[..., list[Any]],  # blender_utils.Object when running in Blender
...
if __name__ == "__main__":
    ...
    # pytorch3d is optional - only needed for this test block
    try:
    ...
    for i, data in enumerate(tqdm(dataloader, dynamic_ncols=True)):
        ...
        # LBS manually (requires pytorch3d)
        if HAS_PYTORCH3D:
            # ! PyTorch3D wants a right-multiplied transformation matrix (applied on row vectors)

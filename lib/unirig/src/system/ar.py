...
class ARWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(self, trainer, pl_module: ARSystem, prediction: List[Dict], batch_indices, batch, batch_idx, dataloader_idx):
        ...
        for (id, detokenize_output) in enumerate(detokenize_output_list):
            ...
            # DEBUG: Show bone names generated by model
            print(f"[ARWriter DEBUG] cls={detokenize_output.cls}, parts={detokenize_output.parts}")
            if detokenize_output.names:
                print(f"[ARWriter DEBUG] Generated {len(detokenize_output.names)} bone names:")
                for i, name in enumerate(detokenize_output.names[:15]):
                    print(f"[ARWriter DEBUG]   {i+1:2d}. {name}")
            else:
                print(f"[ARWriter DEBUG] No bone names generated!")

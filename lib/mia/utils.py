...
def _quaternion_raw_multiply(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """Multiply quaternions (w, x, y, z) using Hamilton product."""
...
def _quaternion_invert(quaternion: torch.Tensor) -> torch.Tensor:
    """Invert a quaternion (w, x, y, z) by negating the imaginary parts."""
...
def matrix_to_ortho6d(matrix: torch.Tensor) -> torch.Tensor:
    """
    Convert rotation matrix to ortho6d representation.

    Args:
        matrix: (..., 3, 3) rotation matrix

    Returns:
        (..., 6) ortho6d representation
    """
    sh = matrix.shape
...
def get_normalize_transform(points: torch.Tensor, keep_ratio=True, recenter=True):
    """Normalize transform using pure PyTorch (NormalizePoints class).
    points: (B, N, 3)
    Note: recenter is always True in this implementation.
    """
    norm = NormalizePoints(keep_ratio=keep_ratio)
    # Initialize the transform by calling transform_points once
    norm.transform_points(points)
    return norm
...
def matrix_to_quat(matrix: Tensor_or_Array) -> Tensor_or_Array:
    ...
    if module is torch:
        # Use local pure PyTorch implementation
...
def quat_transl_to_dualquat(quat: torch.Tensor, transl: torch.Tensor, transl_first=False):
    ...
    """
    # Use local pure PyTorch implementations
    q_r = _standardize_quaternion(F.normalize(quat, dim=-1))
    q_d = 0.5 * _quaternion_raw_multiply(torch.cat([torch.zeros_like(transl[..., :1]), transl], dim=-1), q_r)
...
def dualquat_to_quat_transl(dq: torch.Tensor, concat=False, transl_first=False):
    ...
    if transl_first:
        q_r, q_d = q_d, q_r
    transl = 2.0 * _quaternion_raw_multiply(q_d, _quaternion_invert(q_r))
    transl = transl[..., 1:]
...
def transform_gs(gs: torch.Tensor, transform):
    """Transform Gaussian splatting data.

    Args:
        gs: (N, 14) Gaussian splatting data
        transform: (4, 4) or (B, 4, 4) transformation matrix, or object with get_matrix() method
    """
    gs = gs.clone()
    ...
    if hasattr(transform, 'get_matrix'):
        # Has get_matrix method (e.g. pytorch3d Transform3d or similar)
        matrix = transform.get_matrix().transpose(-1, -2)
    elif isinstance(transform, torch.Tensor):
        matrix = transform
    else:
        matrix = torch.tensor(transform, device=gs.device, dtype=gs.dtype)

    matrix = matrix.to(gs.device)

    # Decompose transformation
    transl, rotation, scaling = decompose_transform(matrix, return_quat=True, return_concat=False)

    # Transform points using homogeneous coordinates
    xyz = gs[..., :3]
    xyz_homo = torch.cat([xyz, torch.ones_like(xyz[..., :1])], dim=-1)
    if matrix.ndim == 3:  # Batched transform
        xyz_homo = xyz_homo.unsqueeze(-2)
        xyz_transformed = torch.einsum('...ij,...j->...i', matrix, xyz_homo)
        xyz_transformed = xyz_transformed.squeeze(-2)
    else:
        xyz_transformed = torch.einsum('ij,...j->...i', matrix, xyz_homo)
    gs[..., :3] = xyz_transformed[..., :3]

    gs[..., 4:7] *= scaling
    gs[..., 7:11] = _quaternion_raw_multiply(rotation, gs[..., 7:11])
